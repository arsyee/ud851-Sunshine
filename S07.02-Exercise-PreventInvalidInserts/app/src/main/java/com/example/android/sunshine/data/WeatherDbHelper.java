    ...
    public void onUpgrade(SQLiteDatabase sqLiteDatabase, int oldVersion, int newVersion) {
        switch (newVersion) {
            case 2:
                ...
                String sqlDropQuery = "DROP TABLE IF EXISTS " + WeatherEntry.TABLE_NAME;
                sqLiteDatabase.execSQL(sqlDropQuery);
                // COMPLETED (4) call onCreate and pass in the SQLiteDatabase (passed in to onUpgrade)
                onCreate(sqLiteDatabase);
                break;
            default:
                Log.e(TAG, "upgrade to unknown version: " + oldVersion + " -> " + newVersion);
                break;
        }
    }
}